...
<template>
  <div>
    ...
    <h1 class="text-2xl font-bold mb-2">Explorador de Correlação de Sinais</h1>
    ...
    <div v-if="result" class="bg-white rounded-lg border border-gray-200 p-6 max-w-lg text-sm">
      <div class="grid grid-cols-2 gap-3 mb-4">
        ...
        <div>
          <div class="text-xs text-gray-400 mb-0.5">Tamanho do efeito</div>
          <div class="tabular-nums">{{ result.effect_size.toFixed(4) }}</div>
        </div>
        <div>
          <div class="text-xs text-gray-400 mb-0.5">Teste utilizado</div>
          <div class="font-mono text-xs">{{ result.test_used }}</div>
        </div>
        <div>
          <div class="text-xs text-gray-400 mb-0.5">Tamanho da amostra</div>
          <div class="tabular-nums">{{ result.sample_size }}</div>
        </div>
        <div>
          <div class="text-xs text-gray-400 mb-0.5">Significativo</div>
          <span
          ...
          <span v-if="result.under_powered" class="ml-2 text-xs text-amber-600 font-medium">baixa potência</span>
        </div>
      </div>
